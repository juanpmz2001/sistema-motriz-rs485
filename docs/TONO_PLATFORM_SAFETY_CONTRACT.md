...
- Authenticated LAN maintenance for diagnostics/telemetry and `STOP ALL` only.
...
- A LAN maintenance client can send diagnostics/telemetry commands and `STOP ALL`, but v1 blocks movement, raw writes, sensitive configuration mutation and destructive OTA commands with `ERR LAN_COMMAND_BLOCKED`.
