...
Kconfig selects `current_robot` or `bench_single_svd48_motor`; the latter is currently
safe-diagnostic only because the unchanged legacy SVD48 driver requires two drives.
Remaining actuation bypasses are individual enable/clear-fault/MOVE_VEL, maintenance
