...
| Actuation | `SET_SPEED`, `ENABLE`, `STOP`, `CLEAR_FAULT`, `MOVE_VEL` |
| Telemetry | `STREAM` |
...
`components/serial_gateway/serial_gateway_policy.c`. Build 19 permits:
...
`control_lan` defines a separate sequenced protocol on default port `32322`, but
build 19 does not initialize or start it. It is not an available device API and
