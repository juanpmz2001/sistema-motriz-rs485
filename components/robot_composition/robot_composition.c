                                  ...
                                  const robot_endpoint_profile_t *endpoint)
{
    ...
    for (size_t device_index = 0; device_index < profile->device_count; ++device_index) {
        ...
        if (device->id == endpoint->device_id) {
            return device->driver_id == ROBOT_DRIVER_SVD48 ? index : UINT8_MAX;
        }
                                 ...
                                 robot_control_handle_t legacy_robot)
{
    ...
    size_t svd48_channel_count = 0;
    for (size_t index = 0; index < profile->device_count; ++index) {
        const robot_device_profile_t *device = &profile->devices[index];
        if (device->driver_id != ROBOT_DRIVER_SVD48) {
            return ESP_ERR_NOT_SUPPORTED;
        }
        svd48_channel_count += device->channel_count;
    }
    if (svd48_channel_count != SVD48_MOTOR_COUNT) {
        return ESP_ERR_NOT_SUPPORTED;
    }
    ...
    for (size_t index = 0; index < profile->endpoint_count; ++index) {
        ...
        const uint8_t motor_index = legacy_motor_index(profile, endpoint);
        if (motor_index >= SVD48_MOTOR_COUNT) {
            robot_composition_deinit(composition);
            return ESP_ERR_NOT_SUPPORTED;
        }
        if (!robot_control_endpoint_adapter_init(
                &composition->adapters[index], legacy_robot, motor_index,
