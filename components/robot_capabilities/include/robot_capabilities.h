...
typedef enum {
    ROBOT_CAP_OK = 0, ROBOT_CAP_INVALID_ARGUMENT, ROBOT_CAP_UNAVAILABLE,
    ROBOT_CAP_UNSUPPORTED, ROBOT_CAP_OUT_OF_RANGE, ROBOT_CAP_IO_ERROR,
} robot_capability_error_t;
typedef enum {
...
} robot_velocity_rpm_ops_t;
struct robot_velocity_rpm_port {
    const robot_velocity_rpm_ops_t *ops;
    void *context;
    int16_t min_rpm;
    int16_t max_rpm;
};
...
typedef struct robot_position_port robot_position_port_t;
typedef struct {
    robot_capability_error_t (*set_position_degrees)(robot_position_port_t *, float);
} robot_position_ops_t;
struct robot_position_port {
    const robot_position_ops_t *ops;
    void *context;
    float min_degrees;
    float max_degrees;
};
...
typedef enum {
    ROBOT_REGISTRY_OK = 0, ROBOT_REGISTRY_INVALID_ARGUMENT,
    ROBOT_REGISTRY_DUPLICATE_ID, ROBOT_REGISTRY_FULL,
} robot_registry_error_t;
